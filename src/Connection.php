...
class Connection extends \PDO {
	...
	public function alterTable(Table &$table) {
		...
		if ($table->shouldDropPrimaryKey(true)) {
			if (!in_array($table->getPrimaryKey(), $dropColumns)) $qry .= $alter . ' DROP PRIMARY KEY;';
		}
		...
		if (count($table->getAlterColumns())) {
			...
			foreach ($cols as $column => $desc) {
				if (in_array($column, $table->getColumns(true))) {
					...
					if ($column == $table->getNewPrimarykey()) $query .= ' PRIMARY KEY FIRST';
	...
	private function addNewColumns(array $columns, Table $table, array $dropColumns = array()) {
		$qry = '';
		$cnt = 1;
