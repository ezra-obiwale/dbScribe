...
class ArrayCollection extends ArrayObject implements JsonSerializable {
	...
	public function __get($name) {
		if (is_object($this->first())) return $this->first()->$name;
	}
	...
	public function current() {
		if (is_int($this->current)) return $this->get($this->current);
	}
	...
	public function remove($value) {
		...
		if (!in_array($value, $oldCollection)) return $this;
	...
	public function find($value) {
		...
		foreach ($this as $object) {
			if ($value === $object) $return->append($object);
		}
		return $return;
	}
	...
	public function findObjectByMethod($method, $value, array $methodArgs = array()) {
		foreach ($this as $object) {
			if (method_exists($object, $method)) {
				if ($value === call_user_func_array(array($object, $method), $methodArgs)) return $object;
			}
		}
	}
	...
	public function findObjects($property, $value) {
		...
		foreach ($this as $object) {
			if (property_exists($object, $property)) {
				if ($value === $object->$property) $return->append($object);
			}
		}
		return $return;
	}
	...
	public function findObject($property, $value) {
		foreach ($this as $object) {
			if (property_exists($object, $property)) {
				if ($value === $object->$property) return $object;
			}
		}
	}
